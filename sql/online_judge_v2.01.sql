...
--
-- 替换视图以便查看 `problem_submit_log`
-- （参见下面的实际视图）
--
DROP VIEW IF EXISTS `problem_submit_log`;
...
--
-- 替换视图以便查看 `user_submit_log`
-- （参见下面的实际视图）
--
DROP VIEW IF EXISTS `user_submit_log`;
...
--
-- 视图结构 `problem_submit_log`
--
DROP TABLE IF EXISTS `problem_submit_log`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY INVOKER VIEW `problem_submit_log`  AS  select `online_judge`.`problem`.`problem_id` AS `problem_id`,count((case when (`online_judge`.`submit`.`status` = 'AC') then `online_judge`.`submit`.`status` end)) AS `ac`,count((case when (`online_judge`.`submit`.`status` = 'WA') then `online_judge`.`submit`.`status` end)) AS `wa`,count((case when (`online_judge`.`submit`.`status` = 'TLE') then `online_judge`.`submit`.`status` end)) AS `tle`,count((case when (`online_judge`.`submit`.`status` = 'MLE') then `online_judge`.`submit`.`status` end)) AS `mle`,count((case when (`online_judge`.`submit`.`status` = 'RE') then `online_judge`.`submit`.`status` end)) AS `re`,count((case when (`online_judge`.`submit`.`status` = 'SE') then `online_judge`.`submit`.`status` end)) AS `se`,count((case when (`online_judge`.`submit`.`status` = 'CE') then `online_judge`.`submit`.`status` end)) AS `ce` from (`online_judge`.`problem` left join `online_judge`.`submit` on((`online_judge`.`submit`.`problem_id` = `online_judge`.`problem`.`problem_id`))) where (`online_judge`.`problem`.`status` = 1) group by `online_judge`.`problem`.`problem_id` ;

-- --------------------------------------------------------

--
-- 视图结构 `user_submit_log`
--
DROP TABLE IF EXISTS `user_submit_log`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY INVOKER VIEW `user_submit_log`  AS  select `t1`.`user_id` AS `user_id`,ifnull(`t2`.`ac`,0) AS `ac`,`t1`.`wa` AS `wa`,`t1`.`tle` AS `tle`,`t1`.`mle` AS `mle`,`t1`.`re` AS `re`,`t1`.`se` AS `se`,`t1`.`ce` AS `ce` from (((select `online_judge`.`submit`.`user_id` AS `user_id`,count((case when (`online_judge`.`submit`.`status` = 'WA') then `online_judge`.`submit`.`status` end)) AS `wa`,count((case when (`online_judge`.`submit`.`status` = 'TLE') then `online_judge`.`submit`.`status` end)) AS `tle`,count((case when (`online_judge`.`submit`.`status` = 'MLE') then `online_judge`.`submit`.`status` end)) AS `mle`,count((case when (`online_judge`.`submit`.`status` = 'RE') then `online_judge`.`submit`.`status` end)) AS `re`,count((case when (`online_judge`.`submit`.`status` = 'SE') then `online_judge`.`submit`.`status` end)) AS `se`,count((case when (`online_judge`.`submit`.`status` = 'CE') then `online_judge`.`submit`.`status` end)) AS `ce` from (`online_judge`.`users` left join `online_judge`.`submit` on((`online_judge`.`submit`.`user_id` = `online_judge`.`users`.`user_id`))) group by `online_judge`.`submit`.`user_id`)) `t1` left join (select `online_judge`.`submit`.`user_id` AS `user_id`,count(distinct `online_judge`.`submit`.`problem_id`) AS `ac` from `online_judge`.`submit` where (`online_judge`.`submit`.`status` = 'AC') group by `online_judge`.`submit`.`user_id`) `t2` on((`t1`.`user_id` = `t2`.`user_id`))) ;
...
ALTER TABLE `notification`
  ...
  ADD CONSTRAINT `notification_ibfk_2` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE;

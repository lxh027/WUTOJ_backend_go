...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY INVOKER VIEW `problem_submit_log`  AS  select `problem`.`problem_id` AS `problem_id`,count((case when (`submit`.`status` = 'AC') then `submit`.`status` end)) AS `ac`,count((case when (`submit`.`status` = 'WA') then `submit`.`status` end)) AS `wa`,count((case when (`submit`.`status` = 'TLE') then `submit`.`status` end)) AS `tle`,count((case when (`submit`.`status` = 'MLE') then `submit`.`status` end)) AS `mle`,count((case when (`submit`.`status` = 'RE') then `submit`.`status` end)) AS `re`,count((case when (`submit`.`status` = 'SE') then `submit`.`status` end)) AS `se`,count((case when (`submit`.`status` = 'CE') then `submit`.`status` end)) AS `ce` from (`problem` left join `submit` on((`submit`.`problem_id` = `problem`.`problem_id`))) where (`problem`.`status` = 1) group by `problem`.`problem_id` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY INVOKER VIEW `user_submit_log`  AS  select `t1`.`user_id` AS `user_id`,ifnull(`t2`.`ac`,0) AS `ac`,`t1`.`wa` AS `wa`,`t1`.`tle` AS `tle`,`t1`.`mle` AS `mle`,`t1`.`re` AS `re`,`t1`.`se` AS `se`,`t1`.`ce` AS `ce` from (((select `submit`.`user_id` AS `user_id`,count((case when (`submit`.`status` = 'WA') then `submit`.`status` end)) AS `wa`,count((case when (`submit`.`status` = 'TLE') then `submit`.`status` end)) AS `tle`,count((case when (`submit`.`status` = 'MLE') then `submit`.`status` end)) AS `mle`,count((case when (`submit`.`status` = 'RE') then `submit`.`status` end)) AS `re`,count((case when (`submit`.`status` = 'SE') then `submit`.`status` end)) AS `se`,count((case when (`submit`.`status` = 'CE') then `submit`.`status` end)) AS `ce` from (`users` left join `submit` on((`submit`.`user_id` = `users`.`user_id`))) group by `submit`.`user_id`)) `t1` left join (select `submit`.`user_id` AS `user_id`,count(distinct `submit`.`problem_id`) AS `ac` from `submit` where (`submit`.`status` = 'AC') group by `submit`.`user_id`) `t2` on((`t1`.`user_id` = `t2`.`user_id`))) ;
